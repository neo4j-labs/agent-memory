...
async def extract_from_documents():
    ...
    # Create extractor with custom schema. GLiNER2Extractor decodes entities
    # and relations in one pass, so no second extractor is needed.
    schema = create_financial_schema()
    extractor = GLiNER2Extractor(ontology=schema)
