...
the hosted NAMS backend throughout, because its system templates and
server-side extraction make the effect visible with the least setup.

[NOTE]
====
`client.ontology` is not NAMS-only. As of v0.7 the same methods run against your
own Neo4j, where versions are stored as `:Ontology` / `:OntologyVersion` nodes
and the active document drives *local* extraction and resolution. For that
workflow, see
...
* Under `strict`, the service enforces the schema and the SDK raises
  `ValidationError`. On bolt the same mode is enforced client-side, by
  `long_term.add_entity` / `add_relationship`.
...
* link:https://github.com/neo4j-labs/agent-memory/tree/main/examples/ontology-lifecycle[`examples/ontology-lifecycle/`] (Python, NAMS)
* link:https://github.com/neo4j-labs/agent-memory/tree/main/typescript/examples/ontology-lifecycle[`typescript/examples/ontology-lifecycle/`] (TypeScript, NAMS)
* link:https://github.com/neo4j-labs/agent-memory/tree/main/examples/ontology-extraction[`examples/ontology-extraction/`] (Python, bolt — keyless)
