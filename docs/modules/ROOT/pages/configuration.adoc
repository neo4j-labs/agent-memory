= Configuration Reference
:page-product: agent-memory
:page-pagination:
:toclevels: 3
:sectanchors:
:icons: font
:source-highlighter: highlight.js

Complete reference for all configuration options in neo4j-agent-memory.

== Configuration Methods

Neo4j Agent Memory supports multiple configuration methods:

1. **Python Configuration** - Direct instantiation of settings objects
2. **Environment Variables** - Using the `NAM_` prefix
3. **Configuration Files** - YAML or JSON files
4. **Mixed** - Combine methods with environment variables taking precedence

== MemorySettings

The main configuration class that contains all settings.

[source,python]
----
from neo4j_agent_memory import MemorySettings

settings = MemorySettings(
    neo4j=Neo4jConfig(...),
    embedding=EmbeddingConfig(...),
    extraction=ExtractionConfig(...),
    resolution=ResolutionConfig(...),
    schema_config=SchemaConfig(...),
    llm=LLMConfig(...),
    memory=MemoryConfig(...),
    search=SearchConfig(...),
)
----

== Neo4j Configuration

Connection settings for Neo4j database.

=== Python Configuration

[source,python]
----
from neo4j_agent_memory import Neo4jConfig
from pydantic import SecretStr

neo4j_config = Neo4jConfig(
    uri="bolt://localhost:7687",
    username="neo4j",
    password=SecretStr("password"),
    database="neo4j",                    # Database name (default: neo4j)
    max_connection_pool_size=50,         # Connection pool size
    connection_timeout=30.0,             # Connection timeout in seconds
    max_transaction_retry_time=30.0,     # Max retry time for transactions
)
----

=== Environment Variables

[source,bash]
----
NAM_NEO4J__URI=bolt://localhost:7687
NAM_NEO4J__USERNAME=neo4j
NAM_NEO4J__PASSWORD=your-password
NAM_NEO4J__DATABASE=neo4j
NAM_NEO4J__MAX_CONNECTION_POOL_SIZE=50
NAM_NEO4J__CONNECTION_TIMEOUT=30.0
----

=== Parameters

[cols="2,1,1,3"]
|===
|Parameter |Type |Default |Description

|`uri`
|str
|`bolt://localhost:7687`
|Neo4j connection URI

|`username`
|str
|`neo4j`
|Authentication username

|`password`
|SecretStr
|Required
|Authentication password

|`database`
|str
|`neo4j`
|Database name

|`max_connection_pool_size`
|int
|50
|Maximum connection pool size

|`connection_timeout`
|float
|30.0
|Connection timeout (seconds)

|`max_transaction_retry_time`
|float
|30.0
|Maximum transaction retry time
|===

== Embedding Configuration

Settings for vector embeddings.

=== Python Configuration

[source,python]
----
from neo4j_agent_memory import EmbeddingConfig, EmbeddingProvider
from pydantic import SecretStr

embedding_config = EmbeddingConfig(
    provider=EmbeddingProvider.OPENAI,
    model="text-embedding-3-small",
    api_key=SecretStr("sk-..."),         # Or use OPENAI_API_KEY env var
    dimensions=1536,                      # Embedding dimensions
    batch_size=100,                       # Batch size for bulk embedding
    device="cpu",                         # Device for local models
)
----

=== Environment Variables

[source,bash]
----
NAM_EMBEDDING__PROVIDER=openai
NAM_EMBEDDING__MODEL=text-embedding-3-small
NAM_EMBEDDING__API_KEY=sk-...
NAM_EMBEDDING__DIMENSIONS=1536
NAM_EMBEDDING__BATCH_SIZE=100
NAM_EMBEDDING__DEVICE=cpu

# Alternative: use standard OpenAI env var
OPENAI_API_KEY=sk-...
----

=== Parameters

[cols="2,1,1,3"]
|===
|Parameter |Type |Default |Description

|`provider`
|EmbeddingProvider
|`OPENAI`
|Embedding provider (OPENAI, SENTENCE_TRANSFORMERS)

|`model`
|str
|`text-embedding-3-small`
|Model name

|`api_key`
|SecretStr
|None
|API key (for OpenAI)

|`dimensions`
|int
|1536
|Embedding dimensions

|`batch_size`
|int
|100
|Batch size for bulk operations

|`device`
|str
|`cpu`
|Device for local models (cpu/cuda)
|===

=== Embedding Providers

[cols="1,2,2"]
|===
|Provider |Models |Notes

|`OPENAI`
|`text-embedding-3-small`, `text-embedding-3-large`, `text-embedding-ada-002`
|Requires API key

|`SENTENCE_TRANSFORMERS`
|`all-MiniLM-L6-v2`, `all-mpnet-base-v2`, etc.
|Runs locally
|===

== Extraction Configuration

Settings for entity extraction pipeline.

=== Python Configuration

[source,python]
----
from neo4j_agent_memory import ExtractionConfig, ExtractorType, MergeStrategy

extraction_config = ExtractionConfig(
    # Extractor type
    extractor_type=ExtractorType.PIPELINE,

    # Pipeline stages
    enable_spacy=True,
    enable_gliner=True,
    enable_llm_fallback=True,

    # Merge strategy
    merge_strategy=MergeStrategy.CONFIDENCE,
    fallback_on_empty=True,

    # spaCy settings
    spacy_model="en_core_web_sm",
    spacy_confidence=0.85,

    # GLiNER2.5 settings ("gliner" means GLiNER2.5 since 0.7)
    gliner_model="fastino/gliner2.5-base-v1",
    gliner_threshold=0.5,
    gliner_relation_threshold=None,
    gliner_device="cpu",
    gliner_schema=None,
    gliner_max_words=384,
    gliner_chunk_overlap=64,
    gliner_overlap_policy=None,
    gliner_extract_attributes=False,
    gliner_quantize=False,
    gliner_compile=False,

    # LLM settings
    llm_model="gpt-4o-mini",

    # Entity types
    entity_types=["PERSON", "ORGANIZATION", "LOCATION", "EVENT", "OBJECT"],

    # Extraction options
    extract_relations=True,
    extract_preferences=True,
    confidence_threshold=0.5,
)
# Batching and streaming are configured at call time on
# ``ExtractionPipeline.extract_batch()`` / ``StreamingExtractor``.
# Relations come from the same GLiNER2.5 pass as the entities, so there is no
# separate relation extractor to enable.
----

==== GLiNER2.5 Fields

[cols="2,1,3", options="header"]
|===
| Field | Default | Description

| `enable_gliner`
| `True`
| Include the GLiNER2.5 stage in the pipeline

| `gliner_model`
| `fastino/gliner2.5-base-v1`
| Checkpoint id or local path. GLiNER v1 ids (`urchade/++*++`,
  `gliner-community/++*++`, `numind/++*++`) are rejected by the extractor with a
  migration hint, not by settings validation.

| `gliner_threshold`
| `0.5`
| Entity confidence floor (0.0-1.0)

| `gliner_relation_threshold`
| `None`
| Confidence floor for decoded relations. `None` keeps whatever the ontology's
  per-relation thresholds selected.

| `gliner_device`
| `cpu`
| Inference device (`cpu` / `cuda` / `mps`)

| `gliner_schema`
| `None`
| Built-in domain template name (`poleo`, `podcast`, `news`, `scientific`,
  `business`, `entertainment`, `medical`, `legal`). Set, it wins over the
  resolved ontology for the GLiNER2.5 stage.

| `gliner_max_words`
| `384`
| Longest input decoded in one pass; longer text is windowed

| `gliner_chunk_overlap`
| `64`
| Word overlap between windows

| `gliner_overlap_policy`
| `None`
| Span-overlap policy for the attribute pass (`flat` / `nested` / `allow` /
  `longest`); `None` keeps the checkpoint default

| `gliner_extract_attributes`
| `False`
| Run the opt-in second pass that recovers enum properties as attributes

| `gliner_quantize`
| `False`
| Load the weights in fp16

| `gliner_compile`
| `False`
| Run the weights through `torch.compile`
|===

The field names keep the `gliner_` prefix so existing configuration and
`NAM_EXTRACTION__GLINER_*` environment variables keep working — but "gliner"
now means GLiNER2.5.

=== Environment Variables

[source,bash]
----
# Extractor type
NAM_EXTRACTION__EXTRACTOR_TYPE=pipeline    # none, llm, spacy, gliner, pipeline

# Pipeline stages
NAM_EXTRACTION__ENABLE_SPACY=true
NAM_EXTRACTION__ENABLE_GLINER=true
NAM_EXTRACTION__ENABLE_LLM_FALLBACK=true

# Merge strategy
NAM_EXTRACTION__MERGE_STRATEGY=confidence  # union, intersection, confidence, cascade
NAM_EXTRACTION__FALLBACK_ON_EMPTY=true

# spaCy settings
NAM_EXTRACTION__SPACY_MODEL=en_core_web_sm
NAM_EXTRACTION__SPACY_CONFIDENCE=0.85

# GLiNER2.5 settings
NAM_EXTRACTION__GLINER_MODEL=fastino/gliner2.5-base-v1
NAM_EXTRACTION__GLINER_THRESHOLD=0.5
NAM_EXTRACTION__GLINER_RELATION_THRESHOLD=0.35
NAM_EXTRACTION__GLINER_DEVICE=cpu
NAM_EXTRACTION__GLINER_SCHEMA=podcast
NAM_EXTRACTION__GLINER_MAX_WORDS=384
NAM_EXTRACTION__GLINER_CHUNK_OVERLAP=64
NAM_EXTRACTION__GLINER_OVERLAP_POLICY=flat
NAM_EXTRACTION__GLINER_EXTRACT_ATTRIBUTES=false
NAM_EXTRACTION__GLINER_QUANTIZE=false
NAM_EXTRACTION__GLINER_COMPILE=false

# LLM settings
NAM_EXTRACTION__LLM_MODEL=gpt-4o-mini

# Entity types (JSON array)
NAM_EXTRACTION__ENTITY_TYPES='["PERSON","ORGANIZATION","LOCATION","EVENT","OBJECT"]'

# Extraction options
NAM_EXTRACTION__EXTRACT_RELATIONS=true
NAM_EXTRACTION__EXTRACT_PREFERENCES=true
NAM_EXTRACTION__CONFIDENCE_THRESHOLD=0.5
----

NOTE: Batching and streaming have no `NAM_EXTRACTION__` variables. `ExtractionConfig` uses `extra="forbid"`, so an unrecognised key raises at construction time rather than being ignored.

=== Extractor Types

[cols="1,3"]
|===
|Type |Description

|`NONE`
|Disable extraction

|`SPACY`
|spaCy statistical NER only

|`GLINER`
|GLiNER2.5 joint entity + relation extraction only

|`LLM`
|LLM-based extraction only

|`PIPELINE`
|Multi-stage pipeline (default)
|===

=== Merge Strategies

[cols="1,3"]
|===
|Strategy |Description

|`UNION`
|Keep all unique entities from all stages

|`INTERSECTION`
|Only keep entities found by multiple extractors

|`CONFIDENCE`
|Keep highest-confidence version (default)

|`CASCADE`
|Use first stage, fill gaps with subsequent stages

|`FIRST_SUCCESS`
|Stop after first successful stage. Available on
`neo4j_agent_memory.extraction.MergeStrategy` when constructing an
`ExtractionPipeline` directly; the `ExtractionConfig.merge_strategy` enum offers
`union`, `intersection`, `confidence` and `cascade` only.
|===

[#schema-configuration]
== Schema Configuration

Settings for the knowledge graph schema.

=== Python Configuration

[source,python]
----
from neo4j_agent_memory.config.settings import SchemaConfig, SchemaModel

schema_config = SchemaConfig(
    model=SchemaModel.POLEO,              # Schema model
    entity_types=None,                    # Custom types (for CUSTOM model)
    enable_subtypes=True,                  # Track entity subtypes
    strict_types=False,                    # Reject unknown types
    custom_schema_path=None,              # Path to an EntitySchemaConfig file
    ontology_path=None,                   # Path to an ontology document
    use_active_ontology=True,             # Adopt the activated stored version
    ontology_template="poleo",            # Final fallback template
    validation_mode=None,                 # "permissive" | "strict" | None
)
----

==== Ontology Fields

[cols="2,1,3", options="header"]
|===
| Field | Default | Description

| `custom_schema_path`
| `None`
| Path to a schema file (`.json`/`.yaml`). Loaded through `load_ontology`, so it
  may hold an `EntitySchemaConfig` or an `OntologyDocument`; either is converted
  on the way in. Consulted after `ontology_path`.

| `ontology_path`
| `None`
| Path to an ontology document (`.json`/`.yaml`). Highest-priority source after
  the explicit `MemoryClient(ontology=...)` keyword.

| `use_active_ontology`
| `True`
| Adopt the ontology version activated in the database
  (`client.ontology.activate`) when no file or explicit document is given.

| `ontology_template`
| `poleo`
| Built-in template used as the final fallback.

| `validation_mode`
| `None`
| Ontology enforcement on the write paths. `strict` rejects undeclared entity
  types and relationship patterns; `permissive` keeps them with a warning.
  `None` defers to the active stored version's mode, then to `strict` if
  `strict_types` else `permissive`.
|===

The effective ontology is resolved once per connection, in this order: the
explicit `MemoryClient(ontology=...)` keyword, `ontology_path`,
`custom_schema_path`, the active stored `:OntologyVersion` (when
`use_active_ontology`), `model=SchemaModel.CUSTOM` plus `entity_types`, and
finally `ontology_template`.

=== Environment Variables

[source,bash]
----
NAM_SCHEMA_CONFIG__MODEL=poleo                   # poleo, legacy, custom
NAM_SCHEMA_CONFIG__ENTITY_TYPES='["PERSON","PRODUCT"]'  # For custom model
NAM_SCHEMA_CONFIG__ENABLE_SUBTYPES=true
NAM_SCHEMA_CONFIG__STRICT_TYPES=false
NAM_SCHEMA_CONFIG__CUSTOM_SCHEMA_PATH=/path/to/schema.json
NAM_SCHEMA_CONFIG__ONTOLOGY_PATH=/etc/ontology.yaml
NAM_SCHEMA_CONFIG__USE_ACTIVE_ONTOLOGY=true
NAM_SCHEMA_CONFIG__ONTOLOGY_TEMPLATE=poleo
NAM_SCHEMA_CONFIG__VALIDATION_MODE=strict        # permissive, strict
----

=== Schema Models

[cols="1,3"]
|===
|Model |Description

|`POLEO`
|Default POLE+O model (Person, Object, Location, Event, Organization)

|`LEGACY`
|Backward-compatible with older versions

|`CUSTOM`
|User-defined entity types
|===

[#resolution-configuration]
== Resolution Configuration

Settings for entity resolution and deduplication.

=== Python Configuration

[source,python]
----
from neo4j_agent_memory.config.settings import ResolutionConfig, ResolverStrategy

resolution_config = ResolutionConfig(
    strategy=ResolverStrategy.COMPOSITE,  # OntologyResolver on bolt
    exact_threshold=1.0,                  # Exact match threshold
    fuzzy_threshold=0.85,                 # Fuzzy match threshold
    semantic_threshold=0.8,               # Semantic similarity threshold
    fuzzy_scorer="token_sort_ratio",      # RapidFuzz scorer
    # -- ontology-aware resolution on the ingestion path (v0.7) --
    resolve_on_ingest=True,               # Resolve mentions while storing a message
    auto_merge_threshold=0.90,            # Merge at or above this score
    review_threshold=0.85,                # Pending SAME_AS between the two bands
    candidate_limit=12,                   # Blocking candidates per mention/bucket
    use_alias_gazetteer=True,             # Use EntityTypeDef.aliases
    use_embedding_blocking=True,          # Also block via the entity vector index
    context_window_chars=90,              # Mention context half-width
    scope="global",                       # "global" | "user"
)
----

==== Ingest-Time Resolution Fields (v0.7)

[cols="2,1,3", options="header"]
|===
| Field | Default | Description

| `resolve_on_ingest`
| `True`
| Resolve extracted mentions against stored entities while storing a message.
  The single opt-out for v0.7's ingest-time resolution — set `False` for the
  pre-v0.7 behaviour (one node per distinct surface form). Only takes effect
  with an `OntologyResolver` (that is, `strategy=COMPOSITE` on bolt).

| `auto_merge_threshold`
| `0.90`
| Score at or above which a mention merges onto the matched entity.
  Overridable per entity type via `EntityTypeDef.resolution_threshold`.

| `review_threshold`
| `0.85`
| Score at or above which the mention is stored as its own node with a pending
  `SAME_AS` edge to the match, for human review. Overridable per entity type
  via `EntityTypeDef.review_threshold`. Must be `<=` `auto_merge_threshold`;
  the inversion is rejected at settings construction.

| `candidate_limit`
| `12`
| Maximum blocking candidates fetched per mention and per bucket. The
  per-episode exact-key query is capped at `candidate_limit x mentions`, and
  never above 200 rows.

| `use_alias_gazetteer`
| `True`
| Use `EntityTypeDef.aliases` as blocking keys and as a `1.0` match rule.

| `use_embedding_blocking`
| `True`
| Also generate candidates from the entity vector index. Needs an embedder;
  without one the embedding score component drops out and the remaining
  weights are renormalized.

| `context_window_chars`
| `90`
| Half-width of the mention context window compared against a candidate's
  stored context or description during scoring.

| `scope`
| `global`
| `global` matches against every stored entity (the pre-v0.7 behaviour, since
  `:Entity` nodes are global). `user` restricts candidates to entities the
  tenant has mentioned, via
  `+(:Conversation {user_identifier})-[:HAS_MESSAGE]->(:Message)-[:MENTIONS]->(:Entity)+`.
|===

`auto_merge_threshold` / `review_threshold` are the single source for both write
paths: the ingestion path uses them directly, and
`LongTermMemory.add_entity` gets them through the resolver delegation (or
explicitly via `DeduplicationConfig.from_resolution_config()`). See
xref:how-to/tune-entity-resolution.adoc[Tune Entity Resolution].

=== Environment Variables

[source,bash]
----
NAM_RESOLUTION__STRATEGY=composite        # none, exact, fuzzy, semantic, composite
NAM_RESOLUTION__EXACT_THRESHOLD=1.0
NAM_RESOLUTION__FUZZY_THRESHOLD=0.85
NAM_RESOLUTION__SEMANTIC_THRESHOLD=0.8
NAM_RESOLUTION__FUZZY_SCORER=token_sort_ratio
NAM_RESOLUTION__RESOLVE_ON_INGEST=true
NAM_RESOLUTION__AUTO_MERGE_THRESHOLD=0.90
NAM_RESOLUTION__REVIEW_THRESHOLD=0.85
NAM_RESOLUTION__CANDIDATE_LIMIT=12
NAM_RESOLUTION__USE_ALIAS_GAZETTEER=true
NAM_RESOLUTION__USE_EMBEDDING_BLOCKING=true
NAM_RESOLUTION__CONTEXT_WINDOW_CHARS=90
NAM_RESOLUTION__SCOPE=global              # global, user
----

=== Resolver Strategies

[cols="1,3"]
|===
|Strategy |Description

|`NONE`
|No resolution

|`EXACT`
|Exact string matching only

|`FUZZY`
|Fuzzy string matching (using rapidfuzz)

|`SEMANTIC`
|Embedding similarity matching

|`COMPOSITE`
|Default. On bolt this builds `OntologyResolver` — type-constrained blocking in
Cypher, the ontology's alias gazetteer and per-type thresholds, three decision
bands and two-pass episode resolution. It composes the fuzzy and semantic
strategies rather than replacing them. Without a connected Neo4j client (NAMS,
or a client built before connect) it falls back to `CompositeResolver`.
|===

== LLM Configuration

Settings for LLM-based operations.

=== Python Configuration

[source,python]
----
from neo4j_agent_memory import LLMConfig, LLMProvider
from pydantic import SecretStr

llm_config = LLMConfig(
    provider=LLMProvider.OPENAI,
    model="gpt-4o-mini",
    api_key=SecretStr("sk-..."),
    temperature=0.0,
    max_tokens=4096,
)
----

=== Environment Variables

[source,bash]
----
NAM_LLM__PROVIDER=openai
NAM_LLM__MODEL=gpt-4o-mini
NAM_LLM__API_KEY=sk-...
NAM_LLM__TEMPERATURE=0.0
NAM_LLM__MAX_TOKENS=4096

# Alternative
OPENAI_API_KEY=sk-...
----

== Memory Configuration

Settings for memory behavior.

=== Python Configuration

[source,python]
----
from neo4j_agent_memory import MemoryConfig

memory_config = MemoryConfig(
    default_conversation_limit=50,                 # Default messages per conversation
    message_embedding_enabled=True,                # Embed messages for similarity search
    preference_confidence_threshold=0.7,           # Confidence floor for preferences
    fact_deduplication_enabled=True,               # Deduplicate facts on ingest
    trace_embedding_enabled=True,                  # Embed reasoning traces for retrieval
    tool_stats_enabled=True,                       # Aggregate tool-call statistics
)
----

=== Environment Variables

[source,bash]
----
NAM_MEMORY__DEFAULT_CONVERSATION_LIMIT=50
NAM_MEMORY__MESSAGE_EMBEDDING_ENABLED=true
NAM_MEMORY__PREFERENCE_CONFIDENCE_THRESHOLD=0.7
NAM_MEMORY__FACT_DEDUPLICATION_ENABLED=true
NAM_MEMORY__TRACE_EMBEDDING_ENABLED=true
NAM_MEMORY__TOOL_STATS_ENABLED=true
----

== Search Configuration

Settings for search operations.

=== Python Configuration

[source,python]
----
from neo4j_agent_memory import SearchConfig

search_config = SearchConfig(
    default_limit=10,                     # Default result limit
    default_threshold=0.7,                # Minimum similarity score
    hybrid_search_enabled=True,           # Combine vector + graph signals
    graph_depth=2,                        # Hops to traverse during graph search
)
----

=== Environment Variables

[source,bash]
----
NAM_SEARCH__DEFAULT_LIMIT=10
NAM_SEARCH__DEFAULT_THRESHOLD=0.7
NAM_SEARCH__HYBRID_SEARCH_ENABLED=true
NAM_SEARCH__GRAPH_DEPTH=2
----

== Geocoding Configuration

Settings for automatic geocoding of LOCATION entities. When enabled, location names are converted to latitude/longitude coordinates stored as Neo4j Point properties, enabling geospatial queries.

=== Python Configuration (Nominatim - Free)

[source,python]
----
from neo4j_agent_memory import GeocodingConfig, GeocodingProvider

# Nominatim is free but rate-limited to 1 request/second
geocoding_config = GeocodingConfig(
    enabled=True,
    provider=GeocodingProvider.NOMINATIM,
    cache_results=True,                   # Cache to avoid repeated API calls
    rate_limit_per_second=1.0,            # Nominatim requires <= 1 req/sec
    user_agent="my-app/1.0",              # Required by Nominatim ToS
)
----

=== Python Configuration (Google Maps - Higher Accuracy)

[source,python]
----
from neo4j_agent_memory import GeocodingConfig, GeocodingProvider
from pydantic import SecretStr

# Google Maps API requires an API key and has usage costs
geocoding_config = GeocodingConfig(
    enabled=True,
    provider=GeocodingProvider.GOOGLE,
    api_key=SecretStr("your-google-api-key"),
    cache_results=True,
)
----

=== Environment Variables

[source,bash]
----
# Enable geocoding
NAM_GEOCODING__ENABLED=true

# Provider selection
NAM_GEOCODING__PROVIDER=nominatim        # nominatim or google

# API key (required for Google)
NAM_GEOCODING__API_KEY=your-google-api-key

# Caching
NAM_GEOCODING__CACHE_RESULTS=true

# Nominatim-specific settings
NAM_GEOCODING__RATE_LIMIT_PER_SECOND=1.0
NAM_GEOCODING__USER_AGENT=my-app/1.0
----

=== Parameters

[cols="2,1,1,3"]
|===
|Parameter |Type |Default |Description

|`enabled`
|bool
|`False`
|Enable automatic geocoding of LOCATION entities

|`provider`
|GeocodingProvider
|`NOMINATIM`
|Geocoding provider (NOMINATIM or GOOGLE)

|`api_key`
|SecretStr
|None
|API key (required for Google)

|`cache_results`
|bool
|`True`
|Cache geocoding results in-memory

|`rate_limit_per_second`
|float
|1.0
|Rate limit for requests (Nominatim requires ≤1)

|`user_agent`
|str
|`neo4j-agent-memory`
|User-Agent header for Nominatim (required by ToS)
|===

=== Geocoding Providers

[cols="1,2,2,2"]
|===
|Provider |Cost |Rate Limit |Notes

|`NOMINATIM`
|Free
|1 request/second
|Uses OpenStreetMap data. Good for most use cases.

|`GOOGLE`
|Pay per use
|50 requests/second
|Higher accuracy, better address parsing. Requires API key.
|===

=== Usage Example

[source,python]
----
from neo4j_agent_memory import (
    MemoryClient,
    MemorySettings,
    GeocodingConfig,
    GeocodingProvider,
)

# Configure with geocoding enabled
settings = MemorySettings(
    geocoding=GeocodingConfig(
        enabled=True,
        provider=GeocodingProvider.NOMINATIM,
    )
)

async with MemoryClient(settings) as client:
    # LOCATION entities are automatically geocoded
    entity, dedup_result = await client.long_term.add_entity(
        "Empire State Building, New York",
        "LOCATION",
    )

    # Get coordinates
    coords = await client.long_term.get_entity_coordinates(entity.id)
    if coords:
        lat, lon = coords
        print(f"Coordinates: {lat}, {lon}")

    # Search for nearby locations (within 5km)
    nearby = await client.long_term.search_locations_near(
        latitude=40.7484,
        longitude=-73.9857,
        radius_km=5.0,
    )

    # Batch geocode existing locations without coordinates
    stats = await client.long_term.geocode_locations()
    print(f"Geocoded {stats['geocoded']} of {stats['processed']} locations")
----

=== Geospatial Queries

Once locations are geocoded, you can run geospatial queries:

[source,python]
----
# Find locations within a bounding box
locations = await client.long_term.search_locations_in_bounds(
    min_lat=40.70,
    max_lat=40.80,
    min_lon=-74.02,
    max_lon=-73.95,
)

# Find locations near a point
nearby = await client.long_term.search_locations_near(
    latitude=40.7484,
    longitude=-73.9857,
    radius_km=10.0,
    limit=20,
)
----

== Enrichment Configuration

Settings for background entity enrichment from external knowledge sources (Wikipedia, Diffbot).

=== Python Configuration

[source,python]
----
from neo4j_agent_memory.config.settings import EnrichmentConfig, EnrichmentProvider

enrichment_config = EnrichmentConfig(
    enabled=True,                                    # Enable enrichment
    providers=[EnrichmentProvider.WIKIMEDIA],        # Providers to use

    # API keys (Diffbot only)
    diffbot_api_key="your-api-key",                  # Or set DIFFBOT_API_KEY env var

    # Rate limiting
    wikimedia_rate_limit=0.5,                        # Seconds between requests
    diffbot_rate_limit=0.2,                          # Seconds between requests

    # Caching
    cache_results=True,                              # Cache results in memory
    cache_ttl_hours=168,                             # Cache TTL (1 week)

    # Background processing
    background_enabled=True,                         # Enable async processing
    queue_max_size=1000,                             # Max queue size
    max_retries=3,                                   # Retry count
    retry_delay_seconds=60.0,                        # Delay between retries

    # Filtering
    entity_types=["PERSON", "ORGANIZATION", "LOCATION", "EVENT"],
    min_confidence=0.7,                              # Minimum confidence threshold

    # API settings
    language="en",                                   # Wikipedia language
    user_agent="neo4j-agent-memory/1.0",             # User-Agent header
)
----

=== Environment Variables

[source,bash]
----
# Enable enrichment
NAM_ENRICHMENT__ENABLED=true

# Providers (JSON array)
NAM_ENRICHMENT__PROVIDERS=["wikimedia", "diffbot"]

# Diffbot API key
NAM_ENRICHMENT__DIFFBOT_API_KEY=your-api-key
# Or use the standard env var
DIFFBOT_API_KEY=your-api-key

# Rate limiting
NAM_ENRICHMENT__WIKIMEDIA_RATE_LIMIT=0.5
NAM_ENRICHMENT__DIFFBOT_RATE_LIMIT=0.2

# Caching
NAM_ENRICHMENT__CACHE_RESULTS=true
NAM_ENRICHMENT__CACHE_TTL_HOURS=168

# Background processing
NAM_ENRICHMENT__BACKGROUND_ENABLED=true
NAM_ENRICHMENT__QUEUE_MAX_SIZE=1000
NAM_ENRICHMENT__MAX_RETRIES=3
NAM_ENRICHMENT__RETRY_DELAY_SECONDS=60.0

# Filtering
NAM_ENRICHMENT__ENTITY_TYPES=["PERSON", "ORGANIZATION", "LOCATION", "EVENT"]
NAM_ENRICHMENT__MIN_CONFIDENCE=0.7

# API settings
NAM_ENRICHMENT__LANGUAGE=en
NAM_ENRICHMENT__USER_AGENT=neo4j-agent-memory/1.0
----

=== Parameters

[cols="2,1,1,3"]
|===
|Parameter |Type |Default |Description

|`enabled`
|bool
|`False`
|Enable the enrichment system

|`providers`
|list[EnrichmentProvider]
|`[WIKIMEDIA]`
|Providers to use (tried in order)

|`diffbot_api_key`
|SecretStr
|None
|API key for Diffbot

|`wikimedia_rate_limit`
|float
|0.5
|Seconds between Wikimedia requests

|`diffbot_rate_limit`
|float
|0.2
|Seconds between Diffbot requests

|`cache_results`
|bool
|`True`
|Cache enrichment results

|`cache_ttl_hours`
|int
|168
|Cache TTL in hours (1 week)

|`background_enabled`
|bool
|`True`
|Enable background processing

|`queue_max_size`
|int
|1000
|Maximum enrichment queue size

|`max_retries`
|int
|3
|Retry count for failures

|`retry_delay_seconds`
|float
|60.0
|Delay between retries

|`entity_types`
|list[str]
|See above
|Entity types to enrich

|`min_confidence`
|float
|0.7
|Minimum confidence threshold

|`language`
|str
|`"en"`
|Wikipedia language code

|`user_agent`
|str
|`"neo4j-agent-memory/1.0"`
|User-Agent header
|===

=== Enrichment Providers

[cols="1,2,2,2"]
|===
|Provider |Cost |Rate Limit |Notes

|`WIKIMEDIA`
|Free
|2 requests/second
|Uses Wikipedia REST API. Good for general entities.

|`DIFFBOT`
|Pay per use
|5 requests/second
|Richer structured data, requires API key.
|===

See the xref:how-to/entities.adoc[Working with Entities] guide for detailed usage documentation.

[#deduplication]
== Deduplication Configuration

Deduplication is configured through `resolution` (above). There is no
`deduplication` section on `MemorySettings` and no `NAM_DEDUPLICATION__*`
environment variables — `resolution.auto_merge_threshold` and
`resolution.review_threshold` are the bands both the message-ingestion path and
`long_term.add_entity` use, and `resolution.strategy` selects the resolver
behind them.

`DeduplicationConfig` is a dataclass, not a settings section: pass it to
`LongTermMemory` when you construct the layer yourself. Its fields are
`enabled`, `auto_merge_threshold` (0.95), `flag_threshold` (0.85),
`use_fuzzy_matching` (True), `fuzzy_threshold` (0.9), `max_candidates` (10) and
`match_same_type_only` (True).

[source,python]
----
from neo4j_agent_memory.memory import DeduplicationConfig, LongTermMemory

# Derive the bands from the resolution settings so the two cannot drift.
long_term = LongTermMemory(
    neo4j_client,
    embedder=embedder,
    deduplication=DeduplicationConfig.from_resolution_config(settings.resolution),
)
----

Per-entity-type overrides live on the ontology (`EntityTypeDef`):
`resolution_threshold` and `review_threshold`. Per-call, `add_entity(...,
deduplicate=False)` skips the check for one entity. See
xref:how-to/deduplication.adoc[Handle Duplicate Entities] and
xref:how-to/tune-entity-resolution.adoc[Tune Entity Resolution].

== Observability Configuration

Settings for tracing and monitoring with OpenTelemetry or Opik.

=== Python Configuration (OpenTelemetry)

[source,python]
----
from neo4j_agent_memory import ObservabilityConfig, TracingProvider

observability_config = ObservabilityConfig(
    enabled=True,
    provider=TracingProvider.OPENTELEMETRY,
    service_name="my-agent-memory",
    endpoint="http://localhost:4317",     # OTLP endpoint
    sample_rate=1.0,                      # Trace all requests
    log_level="INFO",
)
----

=== Python Configuration (Opik)

[source,python]
----
from neo4j_agent_memory import ObservabilityConfig, TracingProvider

observability_config = ObservabilityConfig(
    enabled=True,
    provider=TracingProvider.OPIK,
    project_name="my-agent-memory",
    workspace="my-workspace",             # Optional Opik workspace
    track_llm_calls=True,                 # Track LLM interactions
    track_extraction=True,                # Track extraction pipeline
    track_memory_ops=True,                # Track memory operations
)
----

=== Environment Variables

[source,bash]
----
# Common settings
NAM_OBSERVABILITY__ENABLED=true
NAM_OBSERVABILITY__PROVIDER=opentelemetry  # opentelemetry, opik, auto

# OpenTelemetry settings
NAM_OBSERVABILITY__SERVICE_NAME=my-agent-memory
NAM_OBSERVABILITY__ENDPOINT=http://localhost:4317
NAM_OBSERVABILITY__SAMPLE_RATE=1.0
NAM_OBSERVABILITY__LOG_LEVEL=INFO

# Opik settings
NAM_OBSERVABILITY__PROJECT_NAME=my-agent-memory
NAM_OBSERVABILITY__WORKSPACE=my-workspace
NAM_OBSERVABILITY__TRACK_LLM_CALLS=true
NAM_OBSERVABILITY__TRACK_EXTRACTION=true
NAM_OBSERVABILITY__TRACK_MEMORY_OPS=true

# Opik API (if using cloud)
OPIK_API_KEY=your-api-key
OPIK_WORKSPACE=your-workspace
----

=== Parameters

[cols="2,1,1,3"]
|===
|Parameter |Type |Default |Description

|`enabled`
|bool
|`False`
|Enable observability

|`provider`
|TracingProvider
|`AUTO`
|Tracing provider to use

|`service_name`
|str
|`neo4j-agent-memory`
|Service name for traces

|`sample_rate`
|float
|1.0
|Percentage of requests to trace (0.0-1.0)

|`track_llm_calls`
|bool
|`True`
|Track LLM API calls

|`track_extraction`
|bool
|`True`
|Track extraction pipeline stages

|`track_memory_ops`
|bool
|`True`
|Track memory read/write operations
|===

=== Tracing Providers

[cols="1,2,2"]
|===
|Provider |Description |Required Package

|`OPENTELEMETRY`
|Standard OpenTelemetry tracing
|`pip install neo4j-agent-memory[opentelemetry]`

|`OPIK`
|Comet Opik for LLM observability
|`pip install neo4j-agent-memory[opik]`

|`AUTO`
|Auto-detect available provider
|Whichever is installed
|===

== CLI Configuration

The CLI tool uses environment variables and optional configuration files.

=== Environment Variables

[source,bash]
----
# Neo4j connection (required for memory commands)
NAM_NEO4J__URI=bolt://localhost:7687
NAM_NEO4J__USERNAME=neo4j
NAM_NEO4J__PASSWORD=your-password

# Extraction settings
NAM_EXTRACTION__EXTRACTOR_TYPE=pipeline
NAM_EXTRACTION__ENABLE_GLINER=true

# Output format
NAM_CLI__OUTPUT_FORMAT=json              # json, table, yaml
NAM_CLI__VERBOSE=false
NAM_CLI__COLOR=true
----

=== Configuration File

Create a `.neo4j-memory.yaml` file in your project or home directory:

[source,yaml]
----
# .neo4j-memory.yaml
neo4j:
  uri: bolt://localhost:7687
  username: neo4j
  password: your-password

extraction:
  extractor_type: pipeline
  enable_gliner: true
  gliner_threshold: 0.5

cli:
  output_format: json
  verbose: false
  color: true
----

=== CLI Commands

[source,bash]
----
# Extract entities from text
neo4j-memory extract "John works at Acme Corp in New York"

# Extract with specific schema
neo4j-memory extract --schema poleo "..."

# Extract from file
neo4j-memory extract --input document.txt

# List available schemas
neo4j-memory schemas list

# Show schema details
neo4j-memory schemas show poleo

# Get extraction statistics
neo4j-memory stats

# Output as table
neo4j-memory extract --format table "..."
----

== Complete Example

=== Python Configuration

[source,python]
----
from neo4j_agent_memory import (
    MemorySettings,
    Neo4jConfig,
    EmbeddingConfig,
    EmbeddingProvider,
    ExtractionConfig,
    ExtractorType,
    MergeStrategy,
    SchemaConfig,
    SchemaModel,
    ResolutionConfig,
    ResolverStrategy,
    LLMConfig,
    LLMProvider,
    GeocodingConfig,
    GeocodingProvider,
)
from pydantic import SecretStr

settings = MemorySettings(
    neo4j=Neo4jConfig(
        uri="bolt://localhost:7687",
        username="neo4j",
        password=SecretStr("password"),
    ),
    embedding=EmbeddingConfig(
        provider=EmbeddingProvider.OPENAI,
        model="text-embedding-3-small",
    ),
    extraction=ExtractionConfig(
        extractor_type=ExtractorType.PIPELINE,
        enable_spacy=True,
        enable_gliner=True,
        enable_llm_fallback=True,
        merge_strategy=MergeStrategy.CONFIDENCE,
    ),
    schema_config=SchemaConfig(
        model=SchemaModel.POLEO,
        enable_subtypes=True,
    ),
    resolution=ResolutionConfig(
        strategy=ResolverStrategy.COMPOSITE,
    ),
    geocoding=GeocodingConfig(
        enabled=True,
        provider=GeocodingProvider.NOMINATIM,
        cache_results=True,
    ),
    llm=LLMConfig(
        provider=LLMProvider.OPENAI,
        model="gpt-4o-mini",
    ),
)
# Per-entity deduplication is configured via DeduplicationConfig
# at the memory layer (see how-to/deduplication.adoc).
# Observability tracers are obtained from
# ``neo4j_agent_memory.observability.get_tracer(...)``.
----

=== Environment Variables (`.env` file)

[source,bash]
----
# Neo4j
NAM_NEO4J__URI=bolt://localhost:7687
NAM_NEO4J__USERNAME=neo4j
NAM_NEO4J__PASSWORD=your-password

# Embedding
NAM_EMBEDDING__PROVIDER=openai
NAM_EMBEDDING__MODEL=text-embedding-3-small

# Extraction
NAM_EXTRACTION__EXTRACTOR_TYPE=pipeline
NAM_EXTRACTION__ENABLE_SPACY=true
NAM_EXTRACTION__ENABLE_GLINER=true
NAM_EXTRACTION__ENABLE_LLM_FALLBACK=true
NAM_EXTRACTION__MERGE_STRATEGY=confidence

# Schema
NAM_SCHEMA_CONFIG__MODEL=poleo
NAM_SCHEMA_CONFIG__ENABLE_SUBTYPES=true

# Resolution (deduplication thresholds live here too)
NAM_RESOLUTION__STRATEGY=composite
NAM_RESOLUTION__RESOLVE_ON_INGEST=true
NAM_RESOLUTION__AUTO_MERGE_THRESHOLD=0.90
NAM_RESOLUTION__REVIEW_THRESHOLD=0.85

# Geocoding (for LOCATION entities)
NAM_GEOCODING__ENABLED=true
NAM_GEOCODING__PROVIDER=nominatim
NAM_GEOCODING__CACHE_RESULTS=true
# For Google Maps (instead of Nominatim):
# NAM_GEOCODING__PROVIDER=google
# NAM_GEOCODING__API_KEY=your-google-api-key

# Observability
NAM_OBSERVABILITY__ENABLED=true
NAM_OBSERVABILITY__PROVIDER=opik
NAM_OBSERVABILITY__PROJECT_NAME=my-agent-memory

# LLM
NAM_LLM__PROVIDER=openai
NAM_LLM__MODEL=gpt-4o-mini

# OpenAI API Key
OPENAI_API_KEY=sk-...
----

== Configuration Precedence

When using multiple configuration methods, the precedence is:

1. **Explicit Python arguments** (highest priority)
2. **Environment variables** with `NAM_` prefix
3. **Default values** (lowest priority)

[source,python]
----
import os

# Set environment variable
os.environ["NAM_NEO4J__URI"] = "bolt://env-server:7687"

# This will use the environment variable
settings = MemorySettings()
print(settings.neo4j.uri)  # bolt://env-server:7687

# This will override the environment variable
settings = MemorySettings(
    neo4j={"uri": "bolt://explicit-server:7687"}
)
print(settings.neo4j.uri)  # bolt://explicit-server:7687
----

== Validation

Settings are validated using Pydantic. Invalid configurations raise `ValidationError`:

[source,python]
----
from neo4j_agent_memory import MemorySettings, ExtractionConfig
from pydantic import ValidationError

try:
    settings = MemorySettings(
        extraction=ExtractionConfig(
            gliner_threshold=1.5  # Invalid: must be 0.0-1.0
        )
    )
except ValidationError as e:
    print(f"Configuration error: {e}")
----

== Next Steps

* xref:getting-started.adoc[Get started with neo4j-agent-memory]
* xref:how-to/entity-extraction.adoc[Configure entity extraction]
* xref:explanation/memory-types.adoc[Learn about memory types]
* xref:how-to/integrations/index.adoc[Framework integrations]
* xref:faq.adoc[Frequently asked questions]
