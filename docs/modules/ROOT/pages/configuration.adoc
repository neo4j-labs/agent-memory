...
# LLM settings
NAM_EXTRACTION__LLM_MODEL=gpt-5-mini
...
Settings for LLM-based operations. Neo4j Agent Memory uses litellm to provide provider-agnostic access to hundreds of LLM models.

Supported providers include:
- OpenAI (GPT-5, GPT-5-mini, GPT-5-pro)
- Google Vertex AI (Gemini models)
- Anthropic (Claude models)
- And 100+ others via litellm

See link:https://litellm.vercel.app/docs/providers[LiteLLM Provider Documentation] for the complete list of supported models.

=== Python Configuration

[source,python]
----
from neo4j_agent_memory import LLMConfig
from pydantic import SecretStr
...
The LLM configuration uses litellm's environment variable convention:

[source,bash]
----
# Model name (required)
NAM_LLM__MODEL=gpt-5-mini

# API key (optional - provider uses standard env vars if not set)
NAM_LLM__API_KEY=sk-...
...
# Provider-specific environment variables (detected automatically by litellm)
# OpenAI
OPENAI_API_KEY=sk-...
...
LiteLLM uses a consistent model naming pattern: `provider/model-name`
...
|OpenAI
|No prefix required
|`gpt-5-mini`, `gpt-5-pro`, `gpt-5-nano`
...
For the complete list of supported models and their naming conventions, see link:https://litellm.vercel.app/docs/providers[LiteLLM Provider Documentation].

=== Usage Example

[source,python]
----
from neo4j_agent_memory import MemoryClient, MemorySettings, LLMConfig
from neo4j_agent_memory.config.settings import (
    ExtractionConfig,
    ExtractorType,
)

# Configure with any supported LLM
