...
NOTE: POLE+O applies to both backends. On the bolt backend you can also define custom schemas (`SchemaModel.CUSTOM`); on NAMS, typed domain ontologies are managed server-side via xref:how-to/use-ontologies.adoc[`client.ontology`].
...
While POLE+O is the default, you can refine or replace it. As of v0.7 the
first-class way to do that is an *ontology*: a typed document that names your
entity labels (each mapped onto a POLE+O `pole_type` and an optional `subtype`),
the typed relationships between them, and the extraction and resolution
constraints that go with them. One document drives the extractors, relation
validation, entity resolution and the strict write paths — see
xref:explanation/ontologies.adoc[Ontologies] and
xref:how-to/ontology-driven-extraction.adoc[Drive Extraction From an Ontology].

The `SchemaModel` options below still work, and they are still how you pick the
*base* type vocabulary. What changed is that every path now converges on an
ontology document: `SchemaModel.CUSTOM` plus `entity_types` builds an ad-hoc one
(each name mapped onto POLE+O through `map_label_to_poleo`), and
`custom_schema_path` is loaded through `load_ontology()`, so the file may hold
either an `EntitySchemaConfig` or an `OntologyDocument`.
...
The file above is the legacy `EntitySchemaConfig` shape, and it is still
accepted — `load_ontology()` detects it by its `entity_types[].name` keys and
converts it. To get the v0.7 features (annotation guidelines, typed relationship
endpoints, decoding constraints, resolution aliases) write an ontology document
instead and point `ontology_path` at it, which outranks `custom_schema_path`.
...
Entity resolution merges duplicate entities and links aliases. On bolt it runs
on the ingestion path *by default* as of v0.7, and it never crosses a POLE+O
type — "Apple" the ORGANIZATION and "Apple" the OBJECT embed almost
identically, which is exactly why type-constrained blocking matters:
...
settings = MemorySettings(
    resolution=ResolutionConfig(
        ...
        resolve_on_ingest=True,               # default
        auto_merge_threshold=0.90,            # merge at or above
        review_threshold=0.85,                # pending SAME_AS between the two
    )
)
...
async with MemoryClient(settings) as memory:
    ...
    # Both mentions are PERSON, so they are compared; "Johnny Smith" either
    # merges onto "John Smith" (and is recorded as an alias) or lands in the
    # review band as a pending SAME_AS pair.
----

Set `resolve_on_ingest=False` for the pre-v0.7 behaviour: one node per distinct
surface form. See xref:how-to/tune-entity-resolution.adoc[Tune Entity
Resolution] for the bands, the alias gazetteer and the per-type overrides that
an ontology can declare.
...
GLiNER2.5 works from lowercase labels, which map to POLE+O through
`DEFAULT_LABEL_MAPPING` (an ontology's own labels take precedence):
