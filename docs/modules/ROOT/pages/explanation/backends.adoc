...
.Bolt writes directly to a Neo4j instance you operate; NAMS writes through a hosted workspace with background extraction
image::diagrams/bolt-vs-nams-data-flow.svg["Two stacked panels. Bolt: your application drives client-side embedding and entity-extraction providers and writes through the Bolt driver straight to a Neo4j instance you operate. NAMS: your application calls the hosted workspace over HTTPS; the workspace triggers background, server-side extraction that reaches a not-yet-ready state until the readiness API reports it complete",width=100%,link=self]
