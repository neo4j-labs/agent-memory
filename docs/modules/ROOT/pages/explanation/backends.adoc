...
| Ontologies
| `client.ontology` (v0.7) — versioned `:Ontology` / `:OntologyVersion` nodes in your database, and the document drives local extraction and resolution
| `client.ontology` — typed, versioned, validated domain schemas, enforced server-side
...
| `client.ontology` (list / get / get_active / clone / create / update / activate / delete / import_ / diff / migrate / get_migration)
| Yes (`BoltOntology`, v0.7; local `import_` formats only, synchronous `migrate`) | Yes (`NamsOntology`; every `import_` format, async `migrate` jobs)
| `client.ontology_document` / `client.validation_mode` (the resolved ontology driving local extraction)
| Yes | No (extraction and validation run server-side)
