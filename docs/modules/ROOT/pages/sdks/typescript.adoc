...
[IMPORTANT]
====
This is a Neo4j Labs project: actively maintained and community supported,
but not officially supported. There are no SLAs, backward-compatibility
guarantees or scheduled deprecation commitments; APIs may change without notice.
Use the link:https://community.neo4j.com/[Neo4j Community Forum] or
link:https://github.com/neo4j-labs/agent-memory/issues[GitHub Issues] for support.
====

[NOTE]
====
Artifact check on 13 September 2026: npm's latest `@neo4j-labs/agent-memory` was `0.4.1`, with Node `>=20` metadata and older V1-style middleware declarations. This checkout's Node 22 / V4 lesson setup describes source development, not that published artifact. Use the source bootstrap below until a matching release is verified.
====
...
drawer. The CI example matrix checks source compatibility with mocked services.
