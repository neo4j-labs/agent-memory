...
Each section below is one adapter constructor, grouped by SDK. Most implement both `LLMProvider` and `StructuredExtractor`, or `EmbeddingProvider` alone; `InstructorProvider` implements `StructuredExtractor` only. The closing xref:reference/api/adapters.adoc#_adapter_availability[Adapter availability] section shows which installed extra makes which adapter importable.
