...
| Parsed active document. Python 0.6.0 has a binding-metadata limitation; see <<python-0-6-active-binding>> before restoration. Current TypeScript source reads the authoritative response `version`.
...
[#python-0-6-active-binding]
=== Python 0.6.0 active binding

The REST response's `version` identifies the actual bound revision. Python
`neo4j-agent-memory==0.6.0` does not use that field consistently: its
`get_active()` metadata can report the latest revision when an older revision
is active. Do not use that accessor's `version_id` or `validation_mode` to
capture a rollback target or certify restoration.

The self-contained xref:tutorials/ontology-quickstart.adoc[ontology lesson] uses
`read_active_binding(http)` from `hosted_tutorial_helpers.py` to read
`GET /ontologies/active` directly. It uses the same resolved NAMS settings as
the SDK, validates the authoritative `version` and schema, and stops before
mutation if the binding is incomplete. Use that path with Python 0.6.0;
see xref:how-to/use-ontologies.adoc[the activation recipe] for its imports and
client lifecycle. The helper is example code displayed on those pages, not an SDK method.

Current TypeScript source exposes `versionId`, `revision` and `validationMode`
from the authoritative response. Legacy document-only responses, or responses
with a missing/null `version`, leave optional metadata absent. Partial,
malformed or contradictory supplied metadata is rejected. Follow
xref:sdks/typescript.adoc[the TypeScript source setup]; this is not a statement
that a corresponding npm release is available.
...
* xref:explanation/ontologies.adoc[Ontologies] — the concept: system templates, revisions, and validation modes.
* xref:how-to/use-ontologies.adoc[Use ontologies] — the activation recipe.
* xref:reference/nams-limits.adoc[NAMS limits and behavior] — the import/preview rate limit.
