...
image::diagrams/mcp-server-architecture.svg["Self-hosted MCP client, server profiles and backend-dependent memory operations",width=100%,link=self]
...
Search the selected memory types through the integration layer. With NAMS, pass `memory_types=["messages", "entities"]` to avoid the unsupported default preference search — see the fuller NAMS-limitations note under xref:reference/mcp-tools.adoc#_memory_add_preference[memory_add_preference] below.
...
“Platinum” in source comments names a client conformance tier, not a pricing or support commitment — see xref:reference/nams-limits.adoc[NAMS limits and behavior] for what the conformance tiers mean.
