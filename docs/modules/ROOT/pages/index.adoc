...
|Combine *spaCy, GLiNER2.5, and LLM extractors* in a configurable pipeline, with the ontology's endpoint typing enforced while relations are decoded. 8 domain schemas (podcast, news, medical, legal...). Streaming extraction for 100K+ token documents.
...
A podcast corpus transformed into a searchable knowledge graph with a full-stack AI chat agent, interactive graph visualization, geospatial map view, and Wikipedia-enriched entity cards. The real transcript corpus is not shipped with the repository — `make load-sample` runs against synthetic fixtures.

[cols="^1,^1,^1", options="header"]
|===
|Episodes in the full corpus |Agent tools |Memory types
|299 |28 |3
|===
...
https://lennys-memory.vercel.app[Try Live Demo] | https://github.com/neo4j-labs/agent-memory/tree/main/examples/lennys-memory[View on GitHub]
...
https://github.com/neo4j-labs/agent-memory/tree/main/examples/full-stack-chat-agent[View on GitHub]

=== icon:flask[] Domain Schema Examples

8 specialized extraction schemas for different domains. See how domain schemas improve entity extraction accuracy with type descriptions and confidence tuning.

_Schemas: Podcast · News · Scientific · Business · Medical · Legal · Entertainment · POLE+O_

https://github.com/neo4j-labs/agent-memory/tree/main/examples/domain-schemas[View on GitHub]
...
image::diagrams/extraction-pipeline.png[Multi-stage extraction pipeline: spaCy → GLiNER2.5 → LLM → relation validation → Knowledge Graph,width=360,align=left]
...
|icon:crosshairs[] GLiNER2.5
|Zero-shot entities *and* typed relations in one pass, against your ontology
|Accuracy and graph edges without LLM cost

|icon:magic[] LLM fallback
|High-accuracy extraction via API, plus preferences
|Complex or ambiguous text

|icon:check[] Relation validation
|Drops relations the ontology does not permit
|Keeping merged multi-stage output clean
|===

Merge strategies: `CONFIDENCE`, `UNION`, `INTERSECTION`, `CASCADE`, `FIRST_SUCCESS`.
