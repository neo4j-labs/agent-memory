...
By default a NAMS workspace is backed by a managed Neo4j database that NAMS
provisions and operates for you. You can instead point a workspace at **your own
Neo4j** (`dbMode: external`): NAMS stores the memory graph in your database while
still serving the same REST/MCP surface, and external workspaces don't count
against the managed-workspace cap.

Workspace database management — provisioning, switching managed↔external, and
`.cypher` export/import (`/v1/workspace/database/*`) — is an administrative
operation (admin-scoped REST or the dashboard), not part of the memory client.
See the link:https://memory.neo4jlabs.com/openapi.json[OpenAPI spec] for the
exact endpoints.
