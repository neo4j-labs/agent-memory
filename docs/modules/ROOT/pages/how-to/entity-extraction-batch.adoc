...
* **GLiNER2.5 on GPU**: `batch_size=32, max_concurrency=1` — let the GPU saturate with larger batches rather than many small concurrent runs
* **GLiNER2.5 on CPU**: `batch_size=10, max_concurrency=4` — smaller batches, more parallelism
...
# GLiNER2Extractor.extract_batch() decodes each slice in one forward pass and
# returns a plain list[ExtractionResult] — no max_concurrency, no per-item
# error wrapper.
...
Wrap the extractor in an `ExtractionPipeline` when you want the
`BatchExtractionResult` shape (per-item success flags, `max_concurrency`,
`fail_fast`).
...
pipeline = ExtractionPipeline(
    ...
    merge_strategy=MergeStrategy.UNION,   # Keep all extractions
)
