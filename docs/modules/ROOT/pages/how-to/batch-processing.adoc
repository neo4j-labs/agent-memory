...
* Extraction models configured (GLiNER2.5, spaCy, or LLM)
...
# extract_batch() with progress and per-item error handling lives on the
# pipeline; GLiNER2Extractor.extract_batch() returns a plain list of results.
extractor = ExtractionPipeline(stages=[GLiNER2Extractor.for_schema("business")])
...
# DeduplicationConfig is set on the LongTermMemory instance, not per call.
client.long_term.deduplication = DeduplicationConfig(
...
await client.long_term.register_extractor(
    "gliner2",
    version="2.0.0",
    config={"model": "fastino/gliner2.5-base-v1", "schema": "business"},
)

# Store with provenance
for doc_index, item in enumerate(result.results):
    if not item.success:
        continue
...
# Configure for financial domain. There is no built-in "financial" schema —
# "business" is the closest, or build your own ontology.
extractor = ExtractionPipeline(stages=[GLiNER2Extractor.for_schema("business")])
...
# Balanced
balanced_extractor = GLiNER2Extractor.for_schema("business")

# High precision, slower
accurate_pipeline = ExtractionPipeline(
    stages=[
        GLiNER2Extractor.for_schema("business"),
