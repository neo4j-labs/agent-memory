...
| `user_id`
| `None`
| Scopes *writes* to one tenant (the `:User` edges on messages and
  preferences) and gates the `get_user_preferences` tool. It does *not*
  narrow `search()` — the long-term search APIs take no user filter.
