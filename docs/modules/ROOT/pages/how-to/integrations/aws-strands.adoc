...
== Session Manager (Push-Based Persistence)

The tools described above are *pull-based*: the agent decides when to call
`add_memory` or `search_context`. The `Neo4jSessionManager` is *push-based*:
every turn is persisted automatically, conversation history is restored on
agent restart, and (opt-in) relevant long-term memories are injected into each
user message before the model sees it. Pass it to `Agent(session_manager=...)`.
...
manager = Neo4jSessionManager.for_nams(   # MEMORY_API_KEY from env
    "support-42",
    retrieval_config=Neo4jRetrievalConfig(),  # opt-in context injection
)
...
agent("My name is Jane and I work at Acme Corp.")
----

`for_nams` reads `MEMORY_API_KEY` (and optionally `MEMORY_ENDPOINT`) from the
environment, matching the same conventions as `nams_context_graph_tools()`.

=== Quickstart (Self-Hosted Neo4j)
...
Pass `settings=` and the manager owns the client lifecycle. Alternatively,
pass an already-constructed (but not yet connected) `MemoryClient` via
`memory_client=`; see the loop-binding note under <<_limitations>>.
...
=== Tools and Session Manager Together

The tools and the session manager are complementary — the same posture AWS
AgentCore uses. With a session manager attached:

* `add_memory` is for *extra-conversational facts* (information not part of
  the ongoing turn) — conversation capture is now automatic.
* `search_context` remains the explicit deep-query escape hatch for when you
  want the agent to retrieve memories on-demand rather than relying on
  automatic injection.
...
entities extracted from one session are searchable by all others.
