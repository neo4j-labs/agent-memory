...
* **Redaction.** Fully supported *within* an invocation — the write-behind
  ...
  the backend. After the buffer has been flushed, bolt updates the message
  in place; NAMS logs a `WARNING` that server-side redaction is not possible
  (no message-update endpoint on NAMS).
