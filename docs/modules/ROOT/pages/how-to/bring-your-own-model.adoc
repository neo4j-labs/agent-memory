...
* Install: `pip install 'neo4j-agent-memory[anthropic,openai]==0.6.0'`
...
. If the matching SDK for the `openai`, `anthropic`, or `bedrock` prefix is installed, use its native adapter.
...
See xref:how-to/migrate-to-providers.adoc[the migration guide] for the full list of `llm_provider_from_<framework>` helpers.
...
If you change embedding model after creating data, see xref:how-to/migrate-embedding-model.adoc[Migrate to a new embedding model] for the index-rebuild runbook.
...
| `OpenAIProvider`                              | ✓          | ✓ (strict mode)   |           | Native SDK adapter.
...
| `LiteLLMProvider`                             | ✓          | ✓ (via SAP)       |           | LiteLLM routing.
...
vector dimension stays the same; see xref:how-to/migrate-embedding-model.adoc[the migration runbook].
...
* xref:how-to/migrate-to-providers.adoc[Migrate to pluggable providers] — backward compat and side-by-side examples.

// Preserve historical section links after the procedure was reorganized.
anchor:_tldr_string_shorthand[]
