...
The default `wrapStream` forwards stream parts and launches an assistant write
in the background. It exposes no persistence-completion promise. Draining the
model stream does not establish that NAMS stored the answer; cancellation,
errors and runtime shutdown can leave that write incomplete.
...
See xref:how-to/typescript/edge-runtime.adoc[How-to: deploy to edge runtimes] for details.
