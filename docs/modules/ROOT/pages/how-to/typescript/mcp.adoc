...
suite compiles the complete tutorial server to `dist/server.js`, connects over
stdio, and checks 12 base tools or 13 with the custom tool. It does not require
a live NAMS key. Use the tutorial's returned-id sequence for a separate live check.
