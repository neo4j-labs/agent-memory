...
fire-and-forget and exposes no completion promise. Returning/draining the stream
does not prove persistence before a serverless runtime stops. For a route that
must coordinate storage with its lifecycle, use the
xref:how-to/typescript/edge-runtime.adoc[application-owned write pattern].
...
Current-source example dependency declarations (not a release or live-service verification): `next` 16.3.4,
