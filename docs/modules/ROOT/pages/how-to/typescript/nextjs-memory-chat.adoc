...
[NOTE]
====
npm's published `@neo4j-labs/agent-memory` (`0.4.1`) predates the
`LanguageModelV4Middleware` implementation and the `wrapStream` assistant-message
persistence that this guide's route handler depends on
(`agentMemoryMiddleware` from `@neo4j-labs/agent-memory/middleware/vercel-ai`).
Build the SDK from source and link it locally, below, until a matching release
ships — see xref:sdks/typescript.adoc#source-bootstrap[the source bootstrap] on
the TypeScript SDK page for the same steps.
====
...
Build the SDK from source and link it into your app — the same source
build-and-link path xref:how-to/typescript/vercel-ai.adoc[the Vercel AI SDK
guide] uses — so the app resolves the unreleased middleware and streaming
persistence this guide's code needs:

[source,bash]
----
git clone https://github.com/neo4j-labs/agent-memory.git
(cd agent-memory/typescript && npm ci && npm run build)

npm install ai@^7 @ai-sdk/openai@^4 @ai-sdk/react@^4 @neo4j-nvl/react@^1.2
npm install ./agent-memory/typescript
----

`npm install ./agent-memory/typescript` records a `file:` dependency on the
local build — the same wiring the
link:https://github.com/neo4j-labs/agent-memory/tree/main/typescript/examples/nextjs-memory-chat[runnable
example]'s own `package.json` uses (`file:../..`). Adjust the path if you
cloned the repository somewhere else, and rebuild (`npm run build` in
`typescript/`) after pulling SDK changes.
...
The middleware attempts both writes — the user's message in
`transformParams`, the assistant's in `wrapStream` once the stream finishes. You
write no `addMessage` call in this minimal route. The assistant write is
fire-and-forget, so returning/draining the stream does not prove persistence
before a serverless runtime stops — see
xref:how-to/typescript/edge-runtime.adoc#streaming-memory-persistence[the full
derivation]. For a route that must coordinate storage with its lifecycle, use
the xref:how-to/typescript/edge-runtime.adoc[application-owned write pattern].
...
verification): `next` 16.3.4, `react` 19.3.0, `ai` 7.0.97, `@ai-sdk/openai`
4.0.65, `@ai-sdk/react` 4.0.100, `@chakra-ui/react` 3.37.0, `@neo4j-nvl/react`
1.2.1, Node.js 22 — 2026-09-10.
...
new call fails loudly instead of passing silently. This is the last check before
shipping: a green suite here means the route, the middleware wiring and the
memory persistence contract all hold without needing a live API key.

== See also

* xref:how-to/typescript/vercel-ai.adoc[Integrate with the Vercel AI SDK] — the middleware on its own
