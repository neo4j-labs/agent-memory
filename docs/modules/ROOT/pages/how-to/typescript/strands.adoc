...
`Neo4jMemoryStore.forNams({ name: "graph" })` is the form that builds and
owns its own client from the environment.
...
* `"preference"` → `longTerm.addPreference`
* `"fact"` → `longTerm.addFact` (requires `subject`/`predicate`/`object` in metadata)
