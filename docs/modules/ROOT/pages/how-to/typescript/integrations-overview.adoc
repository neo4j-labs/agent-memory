...
observations + recent messages) into each model call for the selected
conversation and attempts to persist both the
...
See xref:how-to/typescript/vercel-ai.adoc[How-to: integrate with the Vercel AI SDK].

== LangChain JS (TypeScript)

`Neo4jChatMessageHistory` supplies framework-neutral history objects; map them
to LangChain message classes before passing them to LangChain APIs.
`Neo4jEntityRetriever` returns plain `{ pageContent, metadata }` objects;
construct LangChain `Document` instances when a framework API requires them.
...
your agent; see xref:how-to/typescript/mastra.adoc[How-to: store Mastra threads in NAMS].
...
`Neo4jMemoryStore` is a Strands `MemoryStore`: hand it to `MemoryManager`
and long-term entity recall is injected into the agent loop before every
model call — the preferred way to give a Strands agent memory.
...
const store = new Neo4jMemoryStore({ name: "graph", client, userId: "alice" });
const agent = new Agent({ model, memoryManager: new MemoryManager({ stores: [store] }) });
----

Separately, `connectMemoryToAgent` returns the `sessionManager` and
