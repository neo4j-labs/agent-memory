...
[.lead]
NAMS is a hosted backend behind the same `MemoryClient`. The migration is additive — your existing bolt code keeps working unchanged; switching to NAMS usually means one config change.
...
```bash
uv pip install 'neo4j-agent-memory[nams]>=0.5.0'
```
...
| Use `client.short_term.bulk_add_messages(session_id, messages)`.
...
=== `backend="bolt"` — NAMS-only methods raise
...
`client.graph` is being retired in favour of `client.query.cypher`:

* **Now** — `client.graph.execute_read` emits a one-time `DeprecationWarning` per process on bolt.
* **Planned (targeted v0.6.0)** — `client.graph` is removed; `client.query.cypher` becomes the only path for raw Cypher.

See the https://github.com/neo4j-labs/agent-memory/blob/main/CHANGELOG.md[CHANGELOG] for the authoritative schedule.
