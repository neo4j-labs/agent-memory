...
vector dimension are fixed when the database is provisioned (currently Amazon
Titan Text v2 at 1024 dimensions by default, or OpenAI `text-embedding-3-small`
at 1536 where configured). `MemorySettings.embedding` is therefore ignored on
