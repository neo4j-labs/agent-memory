...
Demonstrates the standard ``infer_touched_entities`` pattern: a
...
def infer_touched(
    ...
    Entity types are uppercase strings. The ``:TOUCHED`` MERGE stores
    ``type`` verbatim, so mixed-case types here would create a second
    ``:Entity`` node that ``add_entity`` (which uppercases) can never
    match. Prefer ``EntityRef(id=...)`` when the entity already exists —
